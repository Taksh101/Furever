 ...
 * On invalid creds, re-render login with inline field errors (no flash, no redirect).
 */
/**
...
private function finish($row, string $plainPassword, string $role){
  ...
  private function render($view, $data = []) {
    // expose $errors, $email, $role, etc. as real variables in the view
    extract($data);
  ...
  public function registerUser(){
    ...
    $stmt=$pdo->prepare("INSERT INTO pending_registration (role,name,email,phone,city,password_hash,otp_hash,expires_at) VALUES ('user',?,?,?,?,?, ?, DATE_ADD(NOW(), INTERVAL 10 MINUTE))");
    $stmt->execute([$name,$email,$phone,$city,password_hash($pass,PASSWORD_BCRYPT),password_hash($otp,PASSWORD_BCRYPT)]);
  ...
  public function registerNgo(){
    ...
    $stmt=$pdo->prepare("INSERT INTO pending_registration (role,name,email,phone,city,password_hash,otp_hash,expires_at) VALUES ('ngo',?,?,?,?,?, ?, DATE_ADD(NOW(), INTERVAL 10 MINUTE))");
    $stmt->execute([$name,$email,$phone,$city,password_hash($pass,PASSWORD_BCRYPT),password_hash($otp,PASSWORD_BCRYPT)]);
  ...
  public function verify(){
    ...
    if($role==='ngo'){
      ...
      $ins->execute([$p['name'],$p['email'],$p['password_hash'],$p['phone'],$p['city']]);
    } else {
  ...
  public function resendOtp(){
    ...
    $otp=(string)random_int(100000,999999);
    $pdo->prepare("UPDATE pending_registration SET otp_hash=?, expires_at=DATE_ADD(NOW(), INTERVAL 10 MINUTE), attempts=0 WHERE id=?")->execute([password_hash($otp,PASSWORD_BCRYPT),$p['id']]);
  ...
  public function logout(){ Auth::logout(); Response::redirect('login'); }
}
