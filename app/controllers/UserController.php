...
class UserController
{
    ...
    {
        $user = Session::get('user'); // expect ['role'=>'user', 'user_id'=>..., 'email_verified'=>..., 'status'=>...]
        if (!$user || ($user['role'] ?? null) !== 'user') {
            Response::redirect('auth/login?err=login_required');
            exit;
        }
        if ((int)($user['email_verified'] ?? 0) !== 1) {
            Response::redirect('auth/verify?err=email_unverified');
            exit;
        }
        if (($user['status'] ?? 'inactive') !== 'active') {
            Response::redirect('auth/login?err=user_inactive');
            exit;
        }
    }
